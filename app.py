...
# --- 2. SURGICAL LOGIC: BLOCKING MACRO-ECONOMICS ---
COMMODITIES = [
...
# These keywords FORCE the focus onto safety and away from trade
SAFETY_ONLY = "(seized OR raid OR crackdown OR confiscated OR fake OR 'safety test' OR adulteration OR misbranding OR 'Section 16' OR 'FSSAI alert')"

# STERN BLOCKLIST: These words are banned to prevent price/import/currency news
MACRO_BLOCKER = "-rupee -spike -imports -volume -price -trillion -crore -exchange -market -trade -atmanirbhar -economy -stocks -sensex"

@st.cache_data(ttl=300)
def fetch_surgical_intel(query, limit=150):
    try:
        # location:India + Strict filters
...
# --- 4. DATA ACQUISITION ---
# LEFT: Official Vault
vault_data = fetch_surgical_intel(f"site:fssai.gov.in ({COMM_STR}) ({SAFETY_ONLY} OR circular OR sampling)")
# RIGHT: Enforcement News
intel_data = [e for e in fetch_surgical_intel(f"({COMM_STR}) ({SAFETY_ONLY})") if "fssai.gov.in" not in e.link]
